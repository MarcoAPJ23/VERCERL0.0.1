import { iEquipo} from "./Cl_mEquipo.js";
import Cl_vGeneral, { tHTMLElement } from "./tools/Cl_vGeneral.js";

export default class Cl_vJuego extends Cl_vGeneral {
  private btAgregarEquipo: HTMLButtonElement;
  private divEquiposRegistrados: HTMLDivElement;
  constructor() {
    super({ formName: "juego" });
    this.btAgregarEquipo = this.crearHTMLButtonElement("btAgregarEquipo", {
      onclick: () => this.AgregarEquipo(),
    });
    this.divEquiposRegistrados = this.crearHTMLElement(
      "divEquiposRegistrados",
      {
        type: tHTMLElement.CONTAINER,
        refresh: () => this.mostrarEquiposRegistrados(),
      }
    ) as HTMLDivElement;
  }
  mostrarEquiposRegistrados() {
    this.divEquiposRegistrados.innerHTML = "";
    let juego = this.controlador?.EquiposRegistrados();
    if (!juego) return;
    juego.forEach((equipo: iEquipo) => {
      this.divEquiposRegistrados.innerHTML += `<tr>
            <td>${equipo.nombreEquipo}</td>
            <td>${equipo.nombre1}</td>
            <td>${equipo.cedula1}</td>
            <td>${equipo.nombre2}</td>
            <td>${equipo.cedula2}</td>
            <td>${equipo.nombre3 ? equipo.nombre3 : "" }</td>
            <td>${equipo.cedula3 ? equipo.cedula3 : ""}</td>
            <td>${equipo.nombre4 ? equipo.nombre4 : ""}</td>
            <td>${equipo.cedula4 ? equipo.cedula4 : ""}</td>
        </tr>`;
    });
  }
  AgregarEquipo() {
    let nombreEquipo = prompt("Ingrese el nombre del equipo:");
    if (!nombreEquipo) return;
    let nombre1 = prompt("Ingrese el nombre del jugador 1:");
    if (!nombre1) return;
    let cedula1 = prompt("Ingrese la cédula del jugador 1:");
    if (!cedula1) return;
    let nombre2 = prompt("Ingrese el nombre del jugador 2:");
    if (!nombre2) return;
    let cedula2 = prompt("Ingrese la cédula del jugador 2:");
    if (!cedula2) return;
    let nombre3 = prompt("Ingrese el nombre del jugador 3 (Opcional):");
  
    let cedula3 = prompt("Ingrese la cédula del jugador 3 (Opcional):");
   
    let nombre4 = prompt("Ingrese el nombre del jugador 4 (Opcional):");
 
    let cedula4 = prompt("Ingrese la cédula del jugador 4 (Opcional):");
   

    this.controlador!.AgregarEquipo({
      equipoData: {
        nombreEquipo: nombreEquipo,
        nombre1: nombre1,
        cedula1: +cedula1,
        nombre2: nombre2,
        cedula2: +cedula2,
        nombre3: nombre3 || null,
        cedula3: cedula3 ? +cedula3:null,
        nombre4: nombre4 || null,
        cedula4: cedula4 ? +cedula4 : null,
      },
      callback: (error: string | false) => {
        if (error) alert(error);
        this.refresh();
      },
    });
  }
}
